...
function getcat_atr($cat_id = 0, $where = '')
{
	...
	$sql = 'SELECT a.article_id,a.title ' . ' FROM ' . $GLOBALS['ecs']->table('article') . ' AS a ' . ' WHERE a.is_open = 1 AND article_type=0  AND a.' . $condition . $where . ' ORDER BY a.article_type DESC, a.add_time DESC ' . $limit;
