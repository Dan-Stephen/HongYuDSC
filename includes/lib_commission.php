	...
	foreach ($row as $key => $value) {
		...
		if (empty($value['chargeoff_status'])) {
			...
			if ($value['chargeoff_status'] == 1) {
				...
				$sql = 'SELECT SUM(sbo.return_amount) AS return_amount, SUM(sbo.return_shippingfee) AS return_shippingfee FROM ' . $GLOBALS['ecs']->table('seller_bill_order') . ' AS sbo ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON sbo.order_id = o.order_id' . ' WHERE sbo.bill_id = \'' . $value['id'] . '\'' . $where_order;
				$return_info = $GLOBALS['db']->getRow($sql);

				if ($return_info) {
					$value['return_amount'] = $return_info['return_amount'];
					$value['return_shippingfee'] = $return_info['return_shippingfee'];
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('seller_commission_bill') . ' SET return_amount = \'' . $return_info['return_amount'] . '\', return_shippingfee = \'' . $return_info['return_shippingfee'] . '\' WHERE id = \'' . $value['id'] . '\'';
					$GLOBALS['db']->query($sql);
				}
...
function get_alone_goods_rate($order_id = 0, $ru_id = 0, $order = array())
{
	...
	if ($goods_list) {
		...
		foreach ($row['goods'] as $key => $goods) {
			...
			$row[$key]['should_amount'] = number_format($row[$key]['goods_amount'] * $goods['commission_rate'], 2, '.', '');
			$row[$key]['gain_commission'] = $row[$key]['goods_amount'] - $row[$key]['should_amount'];
