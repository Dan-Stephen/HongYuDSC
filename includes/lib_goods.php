...
function get_floor_ajax_goods2($cat_id = 0, $num = 0, $warehouse_id = 0, $area_id = 0, $goods_ids = '', $user_id = 0)
{
	$leftJoin = '';
	$tag_where = ' WHERE w.enabled = 1';
	
	$table = 'wholesale_cat';
	
	if (0 < $cat_id) {
		$tag_where .= ' AND ' . get_children($cat_id, 4, '', $table, '');
	}
	...
	if (!empty($goods_ids)) {
		$tag_where .= ' AND w.goods_id in (' . $goods_ids . ') ';
	}

	$leftJoin .= ' left join ' . $GLOBALS['ecs']->table('goods') . ' as g on w.goods_id = g.goods_id ';
	$leftJoin .= ' left join ' . $GLOBALS['ecs']->table('wholesale_volume_price') . ' as wv on w.goods_id = wv.goods_id ';

	if ($GLOBALS['_CFG']['review_goods'] == 1) {
		$tag_where .= ' AND w.review_status > 2 ';
	}

	$sql = 'SELECT w.*, g.goods_name, g.goods_id, g.goods_thumb, g.goods_img, MIN(wv.volume_number) AS volume_number, wv.volume_price ' . 'FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w ' . $leftJoin . $tag_where . 'group by w.goods_id ORDER BY w.goods_id DESC';
	if (0 < $num) {
		$sql .= ' LIMIT ' . $num;
	}
	...
	foreach ($goods_res as $idx => $row) {
		...
		$goods_res[$idx]['formated_end_date'] = local_date($GLOBALS['_CFG']['date_format'], $row['end_time']);
		$goods_res[$idx]['small_time'] = $row['end_time'] - $now;
		$goods_res[$idx]['goods_name'] = $row['goods_name'];
		$goods_res[$idx]['goods_price'] = $row['goods_price'];
		$goods_res[$idx]['moq'] = $row['moq'];//jinmengwangluo   群:197198555   
		$goods_res[$idx]['volume_number'] = $row['volume_number'];
		$goods_res[$idx]['volume_price'] = $row['volume_price'];
		$goods_res[$idx]['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb'], true);
		$goods_res[$idx]['goods_img'] = get_image_path($row['goods_id'], $row['goods_img']);
		$goods_res[$idx]['url'] = build_uri('wholesale_goods', array('aid' => $row['act_id']), $row['goods_name']);
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min = 0, $max = 0, $ext = '', $warehouse_id = 0, $area_id = 0, $num = 0, $start = 0)
{
	...
	if ($GLOBALS['_CFG']['open_area_goods'] == 1) {
		$leftJoin .= ' LEFT JOIN ' . $GLOBALS['dsc']->table('link_area_goods') . ' AS lag ON g.goods_id = lag.goods_id ';
		$where .= ' AND lag.region_id = \'' . $area_id . '\' ';
	}
...
function get_floor_ajax_goods($cat_id = 0, $num = 0, $warehouse_id = 0, $area_id = 0, $goods_ids = '', $user_id = 0, $search_type = '')
{
	$leftJoin = '';
