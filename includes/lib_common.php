...
function get_promotion_info($goods_id = '', $ru_id = 0, $goods = array())
{
	...
	if (empty($goods_id)) {
		...
		if ($goods) {
			...
			$category_id = isset($goods['brand_id']) && !empty($goods['brand_id']) ? $goods['brand_id'] : 0;
		}
		else {
	...
	if ($is_volume == 1) {
		...
		if ($is_update_sale < 1) {
			...
			foreach ($order_res as $idx => $val) {
				$sql = 'SELECT SUM(og.goods_number) as goods_number ' . 'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g, ' . $GLOBALS['ecs']->table('order_info') . ' AS o, ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND og.order_id = o.order_id AND og.goods_id = g.goods_id ' . ' AND g.goods_id = \'' . $val['goods_id'] . '\'' . $where;
