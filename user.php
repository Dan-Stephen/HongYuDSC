...
require(ROOT_PATH . '/includes/lib_wholesale.php');/*jinmengwangluo*/
...
if ($action == 'register') {
	...
	if ($c) {
		...
		if($type=='wechat' && empty(get_connect_user($info['user_id']))){
             
            $username = 'wx' . substr(md5($info['user_id']), -5) . substr(time(), 0, 4) . mt_rand(1000, 9999);
			...
			$info_user_id='wechat_'.$info['user_id'];  
			...
			$sql = 'UPDATE ' . $ecs->table('users') . (' SET nick_name = \'' . $info['nickname']. '\', old_user_picture = user_picture, aite_id = \'' . $info_user_id . '\', user_picture = \'' . $info['headimgurl'] . '\' WHERE ' . $field_name);
			$db->query($sql);
			...
			$connect_other = array(
				...
				'user_id' => $user_info['user_id'], 
				'open_id' => $info['user_id'],  
				'profile' => serialize($connect_serialize), 
				'create_at' => gmtime());

			$connect_user_count = get_connect_user($info['user_id']); 
			...
			if ($action == 'account_detail' || $action == 'account_paypoints' || $action == 'account_rankpoints') {
				...
				if (isset($_REQUEST['checkboxes'])) {
					...
					foreach ($_REQUEST['checkboxes'] as $key => $val) {
						$goods = rec_goods($val);
						$goods_info = get_goods_info($goods['goods_id'], 0, 0, array('goods_cause'));
