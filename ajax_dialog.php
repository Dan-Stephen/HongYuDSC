...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_stock') {
	...
	$goodsids = !empty($_REQUEST['goodsids']) ? trim($_REQUEST['goodsids']) : '';
	$activitytype = !empty($_REQUEST['activitytype']) ? trim($_REQUEST['activitytype']) : 'snatch';
	$warehouse_id = empty($_REQUEST['warehouse_id']) ? 0 : intval($_REQUEST['warehouse_id']);
	$area_id = empty($_REQUEST['area_id']) ? 0 : intval($_REQUEST['area_id']);
	$goods_list = array();
	$time = gmtime();
	if ($activitytype == 'is_new' || $activitytype == 'is_best' || $activitytype == 'is_hot') {
		$goods_list = $goods_list = get_floor_ajax_goods(0, 6, $warehouse_id, $area_id, $goodsids, 0, $activitytype);
	}
	else {
		$where = ' WHERE 1 ';
		$leftjoin = '';
