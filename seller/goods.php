	...
	if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
		...
		if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
			...
			if (0 < count($attr_arr)) {
				...
				delete_invalid_goods_attr($attr_group, $goods_id, $goods_model, $region_id);
				...
				foreach ($attr_group as $key => $val) {
					...
					if (!empty($product_info)) {
						...
						$product_info = get_product_info_by_attr($goods_id, $attr_info, $goods_model, $region_id, 1);

						if ($product_info) {
							$group = $product_info;
						}
						else {
							$group = insert_attr_changelog($goods_id, $attr_info, $goods_model, $region_id);
						}
			...
			if ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
				...
				if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
					...
					if ($goods_price < 0 || $goods_price == 0 && $_POST['val'] != $goods_price) {
						...
						if ($exc->edit('shop_price = \'' . $goods_price . '\', market_price = \'' . $price_rate . '\', review_status = \'' . $review_goods . '\', last_update=' . gmtime(), $goods_id)) {
							clear_cache_files();
					...
					$sql = 'SELECT goods_id, goods_name, is_delete, is_real, goods_thumb, user_id, ' . 'goods_img, original_img, goods_desc ' . 'FROM ' . $ecs->table('goods') . (' WHERE goods_id = \'' . $goods_id . '\'');
					$goods = $db->getRow($sql);
