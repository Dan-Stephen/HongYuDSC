...
function get_order_transport($goods_list, $consignee = array(), $shipping_id = 0, $shipping_code = '')
{
	...
	if ($goods_list && $shipping_code != 'cac') {
		$area_shipping = get_goods_area_shipping($goods_list, $shipping_id, $shipping_code);
...
function get_goods_area_shipping($goods_list, $shipping_id = 0, $shipping_code = '')
{
	$tid_arr1 = array();
	...
	foreach ($tid_arr2 as $key => $row) {
		...
		if ($trow && $trow['freight_type'] == 1) {
			$sql = 'SELECT * FROM ' . $prefix . 'goods_transport_tpl WHERE tid = \'' . $key . '\' AND shipping_id = \'' . $shipping_id . '\' LIMIT 1';
