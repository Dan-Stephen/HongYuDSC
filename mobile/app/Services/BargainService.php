...
class BargainService
{
	...
	public function goBargain($bargain_id = 0, $bs_id = 0, $user_id = 0, $warehouse_id = 0, $area_id = 0)
	{
		...
		if ($number) {
			$result = array('error' => 1, 'message' => '您已参与砍价，参加新的活动吧！');
			return $result;
		}

		if ($bargain['target_price'] == $bs_log['final_price']) {
			$result = array('error' => 1, 'message' => '已砍至最低价格，参加新的活动吧！');
			return $result;
		}
		else {
		...
		if ($add) {
			...
			$result = array('error' => 2, 'subtract_price' => $subtract_price, 'final_price' => $final_price, 'rank' => $rank, 'user_name' => $user_name, 'user_picture' => $user_picture, 'message' => '砍价成功');
		}
		else {
