...
class GoodsService
{
	...
	public function getGoodsList($categoryId = 0, $keywords = '', $page = 1, $size = 10, $sortKey = '', $sortVal = '', $warehouse_id = 0, $area_id = 0, $shop_goods = '', $range = '', $brand_id = '', $Self = '')
	{
		...
		$brands = $this->goodsRepository->findBrand($categoryId,$brand,$shop_goods);
		
		$field = array('goods_id', 'goods_name', 'shop_price', 'goods_thumb', 'goods_number', 'market_price', 'sales_volume', 'brand_id',);
		$goods_list = $this->goodsRepository->findBy('category', $categoryId, $page, $size, $field, $keywords, $sortKey, $sortVal,$shop_goods,$range,$brand_id,$Self);


		foreach ($goods_list as $k => $v) {
			$goods_list[$k]['goods_thumb'] = get_image_path($v['goods_thumb']);
			$goods_list[$k]['market_price_formated'] = price_format($v['market_price'], false);
			$goods_list[$k]['shop_price'] = $this->goodsRepository->getGoodsOneAttrPrice($v['goods_id']);
			$goods_list[$k]['shop_price_formated'] = price_format($v['shop_price'], false);
		}
