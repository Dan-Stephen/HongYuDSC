...
class FlowService
{
	...
	public function changeCou($uc_id, $userId)
	{
		$this->defaultAddress = $defaultAddress = $this->addressRepository->getDefaultByUserId($userId);
		$cart_goods_list = $this->arrangeCartGoods($userId);
	...
	public function submitOrder($args)
	{
		...
		if ($order['order_amount'] <= 0) {
			$order['order_status'] = self::OS_CONFIRMED;
			$order['confirm_time'] = $time;
			$order['pay_status'] = self::PS_PAYED;
			$order['pay_time'] = $time;
