...
class TeamService
{
	...
	{
		$result = array('error' => 0, 'goods_img' => '', 'goods_info' => '', 'team_log' => '', 'new_goods' => '', 'goods_properties' => '');
		$time = local_gettime();
	...
	public function teamWait($uid = 0, $team_id = 0, $user_id)
	{
		...
		if ($order_info['order_status'] == 2) {
			return array('error' => 1, 'msg' => '亲，您的拼团订单已取消', 'order_id' => $order_info['order_id']);
		}

		if ($order_info['pay_status'] != 2 && $order_info['order_status'] != 2 && $order_info['order_status'] != 4) {
			return array('error' => 1, 'msg' => '亲，您的拼团订单没有支付', 'order_id' => $order_info['order_id']);
		}
