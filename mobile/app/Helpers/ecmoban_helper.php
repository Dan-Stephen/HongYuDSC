...
function single_show_reply_list($parent_id, $page)
{
	...
	$limit = $reply_comment->limit;/*jinmengwangluo*/
...
function get_user_coupons_list($user_id = '', $is_use = false, $total = '', $cart_goods = false, $user = true, $cart_ru_id = -1, $act_type = 'user')
{
	...
	if ($is_use && isset($total) && $cart_goods) {
		...
		foreach ($res as $key => $row) {
			...
			$sql = 'SELECT c.*, cu.uc_id FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' AS cu ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('coupons') . ' AS c ON cu.cou_id = c.cou_id ' . (' WHERE c.review_status = 3 AND c.cou_end_time > ' . $time . ' AND ' . $time . ' > c.cou_start_time') . ' AND ' . $row['order_total'] . ' >= c.cou_man' . (' AND cu.order_id = 0 AND cu.user_id = \'' . $user_id . '\'') . $cart_where . ' GROUP BY cu.uc_id';
		...
		$sql = ' SELECT c.*, cu.* ' . $select . ' FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' AS cu ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('coupons') . ' AS c ON c.cou_id = cu.cou_id ' . $leftjoin . $where . $cart_where . ' GROUP BY cu.uc_id';
		$res = $GLOBALS['db']->getAll($sql);
...
function get_coupons($uc_id = 0, $select = array())
{
	...
	$sql = ' SELECT ' . $select . ' FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' cu ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('coupons') . ' c ON c.cou_id = cu.cou_id ' . (' WHERE cu.uc_id = \'' . $uc_id . '\' AND cu.user_id = \'') . $_SESSION['user_id'] . ('\' AND c.cou_end_time > ' . $time . ' GROUP BY cu.uc_id LIMIT 1 ');
	return $GLOBALS['db']->getRow($sql);
}
