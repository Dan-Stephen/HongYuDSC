...
function order_fee($order, $goods, $consignee, $type = 0, $cart_value = '', $pay_type = 0, $cart_goods_list = '', $warehouse_id = 0, $area_id = 0, $store_id = 0, $store_type = '')
{
	...
	if ($order['extension_code'] != 'group_buy' && $order['extension_code'] != 'bargain_buy') {
...
{
	$rec_txt = array('普通', '团够', '拍卖', '夺宝奇兵', '积分商城', '预售', '秒杀');
	$where = '1';
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_id = 0, $area_id = 0, $store_id = 0, $take_time = '', $store_mobile = '')
{
	$GLOBALS['err']->clean();
	$_parent_id = $parent;
	$leftJoin = '';
	...
	if (0 < $num) {
		$sql = 'SELECT goods_number FROM ' . $GLOBALS['ecs']->table('cart') . ' WHERE ' . $sess_id . (' AND goods_id = \'' . $goods_id . '\' ') . (' AND parent_id = 0 AND goods_attr = \'' . $goods_attr . '\' ') . ' AND extension_code <> \'package_buy\' ' . ' AND rec_type = \'' . CART_GENERAL_GOODS . ('\' AND group_id=\'\' AND warehouse_id = \'' . $warehouse_id . '\' AND stages_qishu=\'-1\' AND store_id = \'') . $store_id . '\' ';
		$row = $GLOBALS['db']->getRow($sql);
