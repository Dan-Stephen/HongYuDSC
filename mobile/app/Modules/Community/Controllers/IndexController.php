...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	public function actionDetail()
	{
		...
		if ($dis_type == 4) {
			...
			$sql = 'SELECT COUNT(*) as total FROM {pre}discuss_circle WHERE parent_id = \'' . $dis_id . '\' ';
			$reply_count = $GLOBALS['db']->getOne($sql);
		}
	...
	public function actionCommentList()
	{
		if (IS_AJAX) {
			...
			if ($dis_type == 4) {
				...
				$sql = 'SELECT COUNT(*) as total FROM {pre}discuss_circle  WHERE parent_id = \'' . $dis_id . '\' ORDER BY add_time DESC';
				$total = $GLOBALS['db']->getOne($sql);
