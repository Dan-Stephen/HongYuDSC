...
function seckill_goods_results($id = '', $page = 1, $offset = 5, $tomorrow)
{
	...
	$sql = ' SELECT g.goods_id,(select sgr.add_time from ' . $GLOBALS['ecs']->table('seckill_goods_remind') . (' AS sgr where sgr.sec_goods_id = sg.id and sgr.user_id=\'' . $_SESSION['user_id'] . '\') as time, g.goods_thumb, g.shop_price, g.market_price, g.goods_name, sg.id, sg.sec_price, sg.sec_num, sg.sec_limit, stb.begin_time, stb.end_time, s.sec_id, s.acti_title, s.acti_time FROM ') . $GLOBALS['ecs']->table('seckill_goods') . ' AS sg LEFT JOIN ' . $GLOBALS['ecs']->table('seckill_time_bucket') . ' AS stb ON sg.tb_id = stb.id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seckill') . ' AS s ON s.sec_id = sg.sec_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON sg.goods_id = g.goods_id ' . (' WHERE s.is_putaway = 1 AND s.review_status = 3 AND s.begin_time <= \'' . $date_begin . '\' AND stb.id = \'' . $id . '\' ') . $where . ' ORDER BY time desc,stb.begin_time ASC ';
	$counts = $GLOBALS['db']->getAll($sql);
	$counts = is_array($counts) ? count($counts) : 0;
	$sql = ' SELECT g.goods_id,(select sgr.add_time from ' . $GLOBALS['ecs']->table('seckill_goods_remind') . (' AS sgr where sgr.sec_goods_id = sg.id and sgr.user_id=\'' . $_SESSION['user_id'] . '\') as time, g.goods_thumb, g.shop_price, g.market_price, g.goods_name, sg.id, sg.sec_price, sg.sec_num, sg.sec_limit, stb.begin_time, stb.end_time, s.sec_id, s.acti_title, s.acti_time FROM ') . $GLOBALS['ecs']->table('seckill_goods') . ' AS sg LEFT JOIN ' . $GLOBALS['ecs']->table('seckill_time_bucket') . ' AS stb ON sg.tb_id = stb.id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('seckill') . ' AS s ON s.sec_id = sg.sec_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON sg.goods_id = g.goods_id ' . (' WHERE s.is_putaway = 1 AND s.review_status = 3 AND s.begin_time <= \'' . $date_begin . '\' AND stb.id = \'' . $id . '\' ') . $where . ' ORDER BY time desc, stb.begin_time ASC ' . $limit;
