	...
	public function actionArticle()
	{
		if (IS_POST) {
			...
			if (empty($article_msg)) {
				...
				$article_msg = dao('article')->field('article_id,link,title,add_time')->where($where)->order('article_id DESC')->limit($num)->select();
	...
	public function actionProduct()
	{
		if (IS_POST) {
			...
			if (!empty($goods_id)) {
				...
				$goods_cache = md5('goods0' . $goods_id . $number);
				$goods = S($goods_cache);
				...
				$product_cache = md5('cat0' . $cat_id . $user_id . $type . $brand . $number);
