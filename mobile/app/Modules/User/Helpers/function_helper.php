...
function get_coupons_lists($num = 2, $page = 1, $status = 0)
{
	...
	$sql = 'SELECT c.*, cu.*, o.order_sn, o.add_time FROM ' . $GLOBALS['ecs']->table('coupons_user') . ' AS cu LEFT JOIN  ' . $GLOBALS['ecs']->table('coupons') . ' AS c ON c.cou_id = cu.cou_id ' . $left_join . $where . (' AND c.review_status = 3 limit ' . $start . ',' . $num . ' ');
...
function get_all_return_order($order_id = 0)
{
	...
	$sql = 'SELECT o.order_id, o.order_sn, o.add_time, o.extension_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' AND o.order_status  IN (\'1\',\'5\', \'6\', \'7\')  AND o.pay_status  IN (\'2\',\'3\') ' . ' AND o.is_delete = 0 ' . ' AND o.pay_time > ' . $log_time . ' AND o.pay_time < ' . $time . $where . ' group by o.order_id ORDER BY o.add_time DESC';
	$order = $GLOBALS['db']->query($sql);

	foreach ($order as $key => $val) {
		$order[$key]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $val['add_time']);
		$sql = 'SELECT og.order_id, og.goods_name, og.goods_number, og.rec_id, g.goods_id, IF(og.extension_code = \'package_buy\', (SELECT activity_thumb FROM ' . $GLOBALS['ecs']->table('goods_activity') . " WHERE act_id = og.goods_id),g.goods_thumb) as goods_thumb\r\n\t\t\tFROM " . $GLOBALS['ecs']->table('order_goods') . ' AS og' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON  og.goods_id = g.goods_id' . ' WHERE og.order_id = ' . $val['order_id'];
