...
class CategoryRepository implements \App\Contracts\Repositories\Article\CategoryRepositoryInterface
{
	...
	public function article_category_child($parent_id)
	{
		...
		foreach ($res as $key => $row) {
			...
			$arr[$key]['url'] = url('article', array('id' => $row['id'], 'urid' => $ru_id), $row['cat_name']);
