...
class OrderRepository implements \App\Contracts\Repositories\Order\OrderRepositoryInterface
{
	...
	{
		$model = \App\Models\OrderInfo::select('*')->where('user_id', $id)->where('order_status', '<>', OS_CANCELED)->where('main_order_id', '<>', 0);
	...
	{
		$model = \App\Models\OrderInfo::select('*')->where('user_id', $id)->where('main_order_id', '<>', 0)->where('order_status', '<>', OS_CANCELED);
