...
class GoodsRepository implements \App\Contracts\Repositories\Goods\GoodsRepositoryInterface
{
	protected $goods;
	private $field;
	...
	public function findBy($field, $value, $page = 1, $size = 10, $columns = array('*'), $keywords = '', $sortKey = '', $sortVal = '', $shop_goods='', $range = '', $brand = '',$Self = '')
	{
		...
		if($brand > 0){
			$goods->where('brand_id', $brand);
		}
		if($shop_goods == 'best'){
			$goods->where('is_best', 1);
		}
		if($shop_goods == 'new'){
			$goods->where('is_new', 1);
		}
		if($range != ''){
			$pc_range = explode('-',$range);
			if((!empty($pc_range[1])) == ''){
				$min = $pc_range[0] - 150;
				$max = $pc_range[0] + 150;
			}else{
		...
		if((!empty($max)) > 0){
			$goods->where('shop_price', '<=', $max);
		}
		if((!empty($min)) > 0){
			$goods->where('shop_price', '>=', $min);
		}
	...
	//品牌列表
	public function findBrand($categoryId,$brand,$shop_goods)
    {
        $brand = \App\Models\Goods::from('goods as g')->distinct('g.brand_id')->select($brand)->leftjoin('brand as b', 'g.brand_id', '=', 'b.brand_id');
		
        // 分类ID
        if ($categoryId != 0) {
            $brand->where('g.cat_id', $categoryId);
        }
		
		if($shop_goods){
			if($shop_goods == 'best'){
				$brand->where('g.is_best', 1);
			}
			if($shop_goods == 'new'){
				$brand->where('g.is_new', 1);
			}
		}
		
        $res = $brand->where('g.is_delete', 0)
			->where('g.brand_id','>', 0)
			->limit(9)
            ->get()
            ->toArray();
	...
	{
		return \App\Models\GoodsGallery::select('img_url')->where('goods_id', $id)->get()->toArray();
	}
	...
	public function goodsAttrNumber($goods_id, $attr_id, $warehouse_id = 0, $area_id = 0, $store_id = 0)
	{
		...
		$prod = $this->goodsWarehouseNumber($goods_id, $warehouse_id, $area_id, $goods['model_attr'], $store_id);
