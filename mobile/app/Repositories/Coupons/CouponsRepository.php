...
class CouponsRepository implements \App\Contracts\Repositories\Coupons\CouponsRepositoryInterface
{
	public function getCouponsLists($num = 2, $page = 1, $status = 0, $uid)
	{
		$time = gmtime();
		...
		$sql = 'SELECT c.*, cu.*, o.order_sn, o.add_time FROM ' . $prefix . 'coupons_user AS cu LEFT JOIN ' . $prefix . 'coupons AS c ON c.cou_id = cu.cou_id ' . $left_join . $where . (' AND c.review_status = 3 limit ' . $start . ',' . $num . ' ');
		$tab = \Illuminate\Support\Facades\DB::select($sql);
		$result['tab'] = $tab;
		$result['totalpage'] = ceil($total['COUNT(*)'] / $num);
		return $result;
	}
	...
	public function goodsCoupont($id = 0, $ruId, $uid)
	{
		...
		foreach ($total as $key => $val) {
			...
			if ($pick < $total[$key]['cou_user_num']) {
				$total[$key]['pick'] = 1;
			}
			else {
				$total[$key]['pick'] = 2;
	...
	public function UserCoupons($user_id = '', $is_use = false, $total = '', $cart_goods = false, $user = true, $cart_ru_id = -1, $act_type = 'user')
	{
		...
		if ($is_use && isset($total) && $cart_goods) {
			...
			foreach ($res as $key => $row) {
				...
				$sql = 'SELECT c.*, cu.uc_id FROM ' . $prefix . 'coupons_user AS cu ' . ' LEFT JOIN ' . $prefix . 'coupons AS c ON cu.cou_id = c.cou_id ' . (' WHERE c.review_status = 3 AND c.cou_end_time > ' . $time . ' AND ' . $time . ' > c.cou_start_time') . ' AND ' . $row['order_total'] . ' >= c.cou_man' . (' AND cu.order_id = 0 AND cu.user_id = \'' . $user_id . '\'') . $cart_where . ' GROUP BY cu.uc_id';
			...
			$sql = ' SELECT c.*, cu.* ' . $select . ' FROM ' . $prefix . 'coupons_user AS cu ' . ' LEFT JOIN ' . $prefix . 'coupons AS c ON c.cou_id = cu.cou_id ' . $leftjoin . $where . $cart_where . ' GROUP BY cu.uc_id';
