...
class CartRepository implements \App\Contracts\Repositories\Cart\CartRepositoryInterface
{
	const FAR_ALL = 0;
	...
	public function getGoodsInCartByUser($id)
	{
		$cart = \App\Models\Cart::select('cart.*')->with(array('goods' => function($query) {
			$query->select('goods_id', 'cat_id', 'goods_name', 'goods_thumb', 'freight', 'tid', 'goods_weight', 'shipping_fee');
