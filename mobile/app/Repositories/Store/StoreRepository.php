...
class StoreRepository implements \App\Contracts\Repositories\Store\StoreRepositoryInterface
{
	public function all()
	{
	...
	public function store_category($id)
	{
		...
		foreach ($res as $key => $row) {
			$arr[$key] = $row;
			$arr[$key]['url'] = url('merchants_store', array('cid' => $row['cat_id'], 'urid' => $id), $row['cat_name']);
			$arr[$key]['opennew'] = 0;
