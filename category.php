...
if (isset($_REQUEST['id'])) {
...
else if ($_REQUEST['suffix'] != '' && $_REQUEST['cat_id'] !='') {
	$cat_id = intval($_REQUEST['cat_id']);
}
else if (defined('THEME_EXTENSION') && $_REQUEST['suffix'] != '' && $_REQUEST['cat_id'] =='') {
	$suffix = $_REQUEST['suffix'];
	$enableTem = $_REQUEST['templates'];
	
	$dir = ROOT_PATH . 'data/category_Templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix;
	if ($preview == 1) {
		$dir_temp = ROOT_PATH . 'data/category_Templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix . '/temp';
	...
	if(!empty($suffix) && file_exists($dir) && defined('THEME_EXTENSION')){
		require ROOT_PATH . 'categorytop.php';
		exit();
	}
}
else {
...
$cat_select = array('parent_id', 'is_top_style', 'top_style_tpl', 'cat_name', 'cat_icon', 'cat_id', 'is_top_visual');
...
if (isset($_REQUEST['act']) && $_REQUEST['act'] == 'load_more_goods' || !$smarty->is_cached($dwt_name . '.dwt', $cache_id)) {
	...
	if($cat_row['is_top_visual'] == 1){
		if($_REQUEST['suffix']){
			$suffix = trim($_REQUEST['suffix']);
		}else{
			$des = ROOT_PATH . 'data/category_Templates/' . $GLOBALS['_CFG']['template'];
			$top_style_tpl = $cat_row['top_style_tpl'];
			$suffix = get_new_dirName2($top_style_tpl, $des);
		}	
		$dir = ROOT_PATH . 'data/category_Templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix;
		if ($preview == 1) {
			$dir_temp = ROOT_PATH . 'data/category_Templates/' . $GLOBALS['_CFG']['template'] . '/' . $suffix . '/temp';
