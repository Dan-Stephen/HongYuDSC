...
function get_homefloor_cat_content(f_this){
	...
	if(eveval == 0){
		$.ajax({
			type: "POST",
			url: "get_ajax_content.php",
			data: "act=floor_cat_content&cat_id=" + cat_id + "&floor_num=" + floor_num + "&warehouse_id=" + warehouse_id + "&area_id=" + area_id + "&section=" + section + "&goods_ids=" + goods_ids + "&floorcat=" + floorcat + "&seller_id=" + seller_id,
			dataType:'json',
			success: function(data){
					...
					if(floorcat == 2){
						...
						for(var i = 0; i < goods.length; i++){
							var html = '';
								...
								if(section == 'vis_wholesale'){
								 var volume_price = '';
								 if(goods[i].volume_price != ''){
									 volume_price = goods[i].volume_price;
								 }else{
									 volume_price = goods[i].goods_price;
								 }
								 if(identi == 1){
									  html = '<div class="p-img"><a href="'+goods[i].url+'" target="_blank"><img src="'+goods[i].goods_thumb+'"></a></div><div class="p-info"><div class="p-name"><a href="'+goods[i].url+'" target="_blank">'+goods[i].goods_name+'</a></div><div class="p-price" style="font-size:12px;"><span style="color:#8c8c8c;">批发价：</span>￥' + volume_price + '</div></div>';
								 }else{
									  html = '<div class="product"><div class="p-img"><a href="'+goods[i].url+'" target="_blank"><img src="'+goods[i].goods_thumb+'"></a></div><div class="p-name"><a href="'+goods[i].url+'" target="_blank">'+goods[i].goods_name+'</a></div><div class="p-price"><div class="shop-price" style="font-size:12px;"><span style="color:#8c8c8c;">批发价：</span>￥' + volume_price + '</div> </div></div>';
								 }
