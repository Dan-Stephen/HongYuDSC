...
function store_order_list($ru_id = 0, $store_id = 0)
{
	...
	$arr = array();
	$sql = 'SELECT so.*, o.order_id, o.order_sn, o.add_time, o.consignee, o.order_status, o.shipping_status, o.pay_status, so.store_id, ' . '(' . order_amount_field('o.') . ') AS total_fee ' . 'FROM ' . $GLOBALS['ecs']->table('store_order') . ' AS so, ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $where . ' AND o.order_id = so.order_id ' . 'ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
...
{
	$sql = ' SELECT og.order_id, og.goods_id, og.goods_name, og.goods_price, og.goods_number, g.goods_thumb, og.goods_sn, og.goods_attr FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.goods_id = og.goods_id ' . (' WHERE og.order_id = \'' . $order_id . '\' ');
