...
if ($_REQUEST['act'] == 'dialog_warehouse') {
	...
	if ($code) {
		if($template_type == 'category_top'){
			$smarty->assign('template', get_seller_template_info2($code, 0, $theme));
		}else{
	...
	if($template_type == 'category_top'){
		$smarty->assign('temp', 'template_category');
	}else{
		$smarty->assign('temp', $_REQUEST['act']);
	}
	...
	if ($db->query($sql)) {
		...
		clear_cache_files();
		$result['message'] = '添加品牌成功！';
	}
	else {
		$result['error'] = 1;
		$result['message'] = '添加品牌异常！';
	}
	...
	if ($_REQUEST['act'] == 'transport_insert' || $_REQUEST['act'] == 'transport_update') {
		...
		if (100 < $cat['commission_rate'] || $cat['commission_rate'] < 0) {
			$result['error'] = 2;
			$result['message'] = '佣金比率为0-100以内，请重新设置！';
			exit($json->encode($result));
		}
