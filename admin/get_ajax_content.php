...
if ($_REQUEST['act'] == 'get_select_category') {
	...
	$result['cat'] = isset($_REQUEST['cat']) ? $_REQUEST['cat'] : '';
	if($result['cat']){
		$temp = 'navigator_category';
	}else{
		$temp = 'navigator_home';
	}
	$smarty->assign('temp',$temp);
	...
	if (!empty($result['sort'])) {
		foreach ($result['sort'] as $k => $v) {
			...
			$sql = 'SELECT a.article_id, a.content, a.title, a.add_time, a.description, a.file_url, a.open_type ' . ' FROM ' . $GLOBALS['ecs']->table('article') . ' AS a ' . ' WHERE a.is_open = 1 AND article_type=0 ' . $where . ' ORDER BY a.article_type DESC, a.add_time DESC LIMIT 5';
