...
function get_goods_list($filter,$tab = '')
{
	$filter->keyword = json_str_iconv($filter->keyword);
	$where = get_where_sql($filter);
	if($tab != ''){
		$sql = 'SELECT w.*, g.goods_name, g.goods_id, g.goods_thumb, g.goods_img, MIN(wvp.volume_number) AS volume_number, wvp.volume_price FROM ' . $GLOBALS['ecs']->table('wholesale') . ' AS w' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON w.goods_id = g.goods_id ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('wholesale_volume_price') . ' AS wvp ON wvp.goods_id = g.goods_id ' . ' WHERE w.enabled = 1 AND w.review_status = 3';
	}
	else
	{
...
function delete_invalid_goods_attr($attr_group = array(), $goods_id = 0, $goods_model = 0, $region_id = 0)
{
	$admin_id = get_admin_id();
