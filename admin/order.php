...
function order_list($page = 0)
{
	...
	if ($result === false) {
		...
		$count_arr = array();
		$status_arr = array(-1, 0, 100, 101, 104);
		...
		if (!empty($filter['keywords'])) {
			...
			foreach ($status_arr as $status) {
				switch ($status) {
				case CS_AWAIT_PAY:
					$count_where = order_query_sql('await_pay', $alias);
					break;

				case CS_AWAIT_SHIP:
					$count_where = order_query_sql('await_ship', $alias);
					break;

				case CS_CONFIRM_TAKE:
					$count_where = order_query_sql('confirm_take', $alias);
					break;

				default:
					if ($status != -1) {
						$count_where = ' AND o.order_status = \'' . $status . '\' ';
					}
				}

				$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $where . $count_where . $where_store . $no_main_order . ' GROUP BY o.order_id';
				$count_arr[$status] = count($GLOBALS['db']->getAll($sql));
			}
		}
		else {
			...
			foreach ($status_arr as $status) {
				...
				$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $where . $count_where . $where_store . $no_main_order;
...
if ($_REQUEST['act'] == 'order_download') {
	...
	$file = MOBILE_WECHAT . '/Controllers/Index.php';
	...
	if ('user' == $step) {
		...
		$consignee = array('country' => $order_info['country'], 'province' => $order_info['province'], 'city' => $order_info['city'], 'district' => $order_info['district'], 'street' => $order_info['street']);
					...
					if (!empty($cart_goods[0]['freight']) && $cart_goods[0]['is_shipping'] == 0) {
						if ($cart_goods[0]['freight'] == 1) {
							...
							if ($trow['freight_type']) {
								...
								$transport_tpl = get_goods_transport_tpl($cart_goods[0], $region, $shipping, $cart_goods[0]['goods_number']);
	...
	array('href' => 'order.php?act=list&' . list_link_postfix(), 'text' => $_LANG['return_list'])
		...
		if ('confirm' == $operation) {
			...
			if ($GLOBALS['_CFG']['use_storage'] == '1' && $GLOBALS['_CFG']['stock_dec_time'] == SDT_PAID) {
				change_order_goods_storage($order_id, false, SDT_PAID, 3);
			}
