...
function merchants_commission_list()
{
	...
	for ($i = 0; $i < count($row); $i++) {
		if (file_exists(MOBILE_DRP)) {
			$is_drp_settlement = merchants_is_settlement($row[$i]['user_id'], 1, $filter);
			$no_drp_settlement = merchants_is_settlement($row[$i]['user_id'], 0, $filter);
			$is_settlement = $is_drp_settlement['all_price'];
			$no_settlement = $no_drp_settlement['all_price'];
		}
		else {
			$is_settlement = merchants_is_settlement($row[$i]['user_id'], 1, $filter);
			$no_settlement = merchants_is_settlement($row[$i]['user_id'], 0, $filter);
		}
...
function merchants_order_list($page = 0)
{
	...
	for ($i = 0; $i < $count; $i++) {
		...
		if ($account_log && $account_log['percent_value']) {
			...
			if ($bill_id) {
				$sql = 'SELECT proportion FROM ' . $GLOBALS['ecs']->table('seller_commission_bill') . (' WHERE id = \'' . $bill_id . '\'');
				$percent_value = $GLOBALS['db']->getOne($sql, true);
