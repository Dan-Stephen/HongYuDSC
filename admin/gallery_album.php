...
function get_pzd_list($ru_id)
{
	...
	if ($result === false) {
		...
		$where .= !empty($filter['seller_list']) ? ' AND ga.ru_id > 0 ' : ' AND ga.ru_id = 0 ';
...
if ($_REQUEST['act'] == 'list') {
	...
	if (0 < $parent_id) {
		...
		$smarty->assign('action_link1', array('text' => $_LANG['return_to_superior'], 'href' => 'gallery_album.php?act=list&parent_id=' . $parent_album_id));
	}
